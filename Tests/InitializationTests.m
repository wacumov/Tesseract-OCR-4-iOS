...
#elif TARGET_OS_MAC
- (void*)pixForImage:(NSImage *)image;
#endif
@end
...
//                           kG8ParamTessdataManagerDebugLevel  : @"1",
    ...
    void (^checkVariablesAreSetForTesseract)(G8Tesseract *tesseract) = ^(G8Tesseract *tesseract){
        // these variables could be set up during the initialization
...
//        [[[tesseract variableValueForKey:kG8ParamTessdataManagerDebugLevel] should] equal:@"1"];
    ...
    void (^recognizeSimpleImageWithTesseract)(G8Tesseract *tesseract) = ^(G8Tesseract *tesseract) {
        tesseract.image = [XPlatformImage imageWithName:@"image_sample.jpg"];
        [tesseract recognize];
    ...
    context(@"Should check common functions", ^{
        ...
        it(@"Should not raise on cache clearing", ^{
            for (int i = 0; i <= 10; i++) {
                ...
                operation.tesseract.image = [XPlatformImage imageWithName:@"well_scanned_page.png"];
        ...
        it(@"Should test pixForImage with zero image size", ^{
            ...
            #elif TARGET_OS_MAC
            tesseract.image = [[NSImage alloc] init];
            #endif
        ...
        it(@"Should set original image if pixForImage for preprocessed image retuens nil", ^{
            ...
            #if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
            UIImage *inputImage = [UIImage imageWithName:@"image_sample.jpg"];
            #elif TARGET_OS_MAC
            NSImage *inputImage = [NSImage imageWithName:@"image_sample.jpg"];
            #endif
...
//                        [[[tesseract variableValueForKey:kG8ParamTessdataManagerDebugLevel] should] equal:@"1"];
                ...
                it(@"Should restore image, rect, sourceResolution and pageSegmentationMode", ^{
                    #if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
                    UIImage *testImage = [UIImage imageWithName:@"image_sample.jpg"];
                    #elif TARGET_OS_MAC
                    NSImage *testImage = [NSImage imageWithName:@"image_sample.jpg"];
                    #endif
